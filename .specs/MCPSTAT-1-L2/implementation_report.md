...
与飞书 L3 方案 revision 9 对齐后，新增一项明确未覆盖边界：当用户 Query 没有产生任何 MCP `tools/call` 时，当前标准 MCP 链路不能观测该 Query，也不能生成 `zero_call/unmatched` 结算。这部分需要宿主提供显式轮次结束契约，因此 L2 的已实现范围不包含零调用 Query 采集。L2 当前也不产生“所属 Skill”分类，该职责明确属于后续分析层。
