...
| 飞书同步基线 | revision 8（2026-08-14 读取） |
| 后续路径 | 契约验收 |
| 创建时间 | 2026-08-07 |
| 最后更新 | 2026-08-14 |
| 当前状态 | 已冻结 |
...
L2 向后续分析仅交付已结算的 Query 和真实调用事实，不判断 Query 属于哪个 Skill。为了让后续分析能发现尚未覆盖的需求，宿主还需要在用户问题没有调用任何 MCP 时显式交付零调用或未命中的轮次结束事件。标准 `tools/call` 本身无法观测这类请求，因此在宿主契约补齐前，系统不得伪造零调用记录。
...
10. 分析层收到的是完整 Query、实际 MCP 调用路径、结果和行为信号，不包含“所属 Skill”分类结论。
11. 宿主能够提交零调用或未命中 MCP 的轮次结束事件；未提供该契约时，平台明确记录这一采集盲区。
...
- 不根据已尝试的 Skill、Tool 名或操作类型推断 Query 类别；语义聚类和 Skill 覆盖缺口识别属于后续分析层。
- 不在只有标准 `tools/call` 的情况下猜测零调用 Query；该类数据必须由宿主提供可验证的轮次结束事件。
...
L2 结算事件仅保存运行时已尝试的 Skill ID 作为客观事实，不将它解释为分类真值。分析层按 `event_id` 以及 `turn_id + settlement_revision` 幂等消费，再独立完成 Query 聚类。零调用输入需要新的宿主轮次结束契约，该契约在冻结前不纳入当前网关调用接口。
...
| D8 | L2 与分析层边界 | L2 交付已结算事实，不判断所属 Skill；零调用 Query 只能由宿主结束事件提供 | 1、3、6、9 | L3 飞书方案 revision 9 |
...
| 零调用 Query 不可见 | 用户未调用任何 MCP，宿主也未提交轮次结束事件 | 后续分析无法发现未覆盖需求 | 明确为上游契约依赖，不从 `tools/call` 或连接关闭猜测 |
