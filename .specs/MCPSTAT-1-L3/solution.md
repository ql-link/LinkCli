...
| 当前状态 | 用户已确认按本方案直接施工；阈值作为可配置默认值，后续根据影子运行数据调整 |
...
当前代码已经具备 L1 接入能力和调用级 L2 信封，但现有 `CallEnvelope` 仍是一条工具调用一条记录，只包含 `projectKey`、`toolName` 等调用事实，尚不能直接表达“一轮 Query 的完整调用过程”。

因此 L3 落地前必须先满足两个输入前提：
...
每条调用事实通过 MCP 注册快照解析为 `project_id/module_id/tool_id`。为避免 MCP 后续改名导致历史数据漂移，L2 需要在结算时保存调用发生当时的模块标识与版本。
...
首版使用以下默认配置，所有数值均可配置，并根据影子运行数据调整：
...
- **已有 Skill 已覆盖**：类别样本已经实际尝试现有 Skill 且没有覆盖缺口时，不重复生成新 Skill 候选。
...
初始容量按日均 10 万轮、峰值 20 QPS、180 天在线证据设计。L2 只负责持续写入，L3 默认每 5 分钟拉取最多 1000 条未分析记录；正常记录先按模块路径缩小语义检索范围，避免全库两两比较。单批失败不标记已分析，由下一批重试。
...
1. 先定义并冻结 L2 完整轮次与零调用记录的兼容输入契约；当前网关不从单次 `CallEnvelope` 猜测整轮边界。
2. 使用已确认的默认阈值实现首版，配置保留调整能力。
3. 实现输入幂等、模块路径和基础数据表。
4. 实现宽口径聚类、场景归纳和周期修正。
...
- MCP 注册信息中稳定的 `module_id` 由 L2 完整轮次输入携带；当前网关注册表没有 Module 实体，L3 不通过 Tool 名猜测模块。
- 首版使用本地、确定性的文本归一化与字符特征相似度，接口保留替换语义模型的能力，不调用外部 LLM。
- 频次、用户数、时间跨度、成功率和覆盖缺口使用本文默认值，并通过配置调整。
- L4 Skill 元数据读取、Skill 生成和数据库反向校验不在本次 L3 代码范围内；本次只落地候选 Outbox 与反馈输入契约。
- 当前没有可消费的完整轮次 L2 表，因此本次同时提供幂等分析输入表和写入接口；L2 接入前，L3 定时任务可以运行但不会产生候选。

这些边界不改变核心结论：**L2 实时记录事实，L3 定时批量完成宽口径 Query 聚类与统计，L4 负责生成或扩展 Skill 并验证其正确性。**
