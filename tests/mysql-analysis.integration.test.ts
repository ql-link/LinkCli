...
const embeddings:EmbeddingProvider={modelVersion:"test:mysql-semantic:2",async embed(texts){return texts.map(()=>[1,0]);}};
const judge:ClusterJudge={modelVersion:"test:mysql-judge",async assign({candidates}){return{clusterId:candidates[0]?.clusterId??null,confidence:1,reason:"test"};},async shouldMerge(){return{sameDemand:true,confidence:1,reason:"test"};}};
...
realMySqlDescribe("L3 real MySQL scheduled batch",()=>{
  ...
  it("persists one broad cluster, three scenes and one idempotent L4 candidate",async()=>{
    ...
    const service=new AnalysisBatchService(repository,embeddings,judge,{minimumSamples:3,minimumActors:3,minimumSpanMs:2*24*60*60*1_000,minimumInputCompleteness:1,minimumCohesion:0.8,minimumSuccessRate:0.9});
  ...
  it("emits one expansion after an earlier new Skill candidate develops a coverage gap",async()=>{
    const repository=new MySqlAnalysisRepository(pool,pool); const consumer=new AnalysisInputConsumer(repository);
    const thresholds={minimumSamples:1,minimumActors:1,minimumSpanMs:0,minimumInputCompleteness:1,minimumCohesion:0,minimumSuccessRate:0,minimumCoverageGapCount:1,minimumCoverageGapRatio:1};
