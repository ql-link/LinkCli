import { queryFingerprint, sha256, querySimilarity, sceneOf } from "./similarity.js";
...
export class AnalysisBatchService {
  ...
  constructor(private readonly repository: AnalysisRepository, thresholds: Partial<ClusterThresholds> = {}, private readonly coverage: SkillCoverageResolver = new NoopSkillCoverageResolver()) {
  ...
  private async processInput(repository: AnalysisRepository, input: AnalysisInput, now: Date): Promise<{ analyzed: boolean; skipped: boolean; candidate: boolean }> {
    ...
    const ranked = candidates.map((cluster) => ({ cluster, similarity: input.queryFingerprint === queryFingerprint(cluster.representativeQuery) ? 1 : querySimilarity(input.queryText, cluster.representativeQuery) })).sort((a, b) => b.similarity - a.similarity);
    ...
    if (!cluster) {
      ...
      cluster = await repository.createCluster({ clusterKey:sha256(`${type}\0${input.projectScope ?? ""}\0${input.modulePathHash ?? ""}\0${input.queryFingerprint}`),clusterType:type,projectScope:input.projectScope,modulePathHash:input.modulePathHash,modulePath:input.modulePath,
        representativeEventId:input.eventId,representativeQuery:input.queryText,status:"observing",sampleCount:0,distinctActorCount:0,successCount:0,coverageGapCount:0,attemptedSkillCount:0,semanticCohesion:0,inputCompleteness:0,firstSeenAt:input.occurredAt,lastSeenAt:input.occurredAt,version:0 });
    }
    const scene = sceneOf(input.calls);
    const qualitySuccess = isQualitySuccess(input);
    const inserted = await repository.addMember({ clusterId:cluster.id,analysisInputId:input.id,semanticSimilarity:similarity,sceneType:scene?.type??null,thresholdEligible:true,qualitySuccess,exclusionReason:null });
