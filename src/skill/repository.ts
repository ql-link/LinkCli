...
export class MemorySkillRepository implements SkillRepository {
  ...
  async listValidationRuns(skillId: string): Promise<SkillValidationRun[]> { return clone([...this.validationRuns.values()].filter((run) => run.skillId === skillId).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())); }
  ...
  async enqueueValidation(job: SkillValidationJob): Promise<boolean> { if (await this.findActiveValidationJob(job.skillVersionId, job.trigger)) return false; this.jobs.set(job.id, clone(job)); return true; }
  async findActiveValidationJob(skillVersionId: string, trigger: ValidationTrigger): Promise<SkillValidationJob | null> { return clone([...this.jobs.values()].find((item) => item.skillVersionId === skillVersionId && item.trigger === trigger && ["pending", "running"].includes(item.status)) ?? null); }
  async listValidationJobs(skillId: string): Promise<SkillValidationJob[]> { return clone([...this.jobs.values()].filter((job) => job.skillId === skillId).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())); }
...
export class MySqlSkillRepository implements SkillRepository {
  ...
  async listValidationRuns(skillId: string): Promise<SkillValidationRun[]> { const [rows] = await this.executor.query<RowDataPacket[]>("SELECT * FROM mcp_skill_validation_runs WHERE skill_id=? ORDER BY created_at DESC,id DESC", [skillId]); return rows.map(validationFrom); }
  ...
  async enqueueValidation(job: SkillValidationJob): Promise<boolean> { try { const [result] = await this.executor.execute<ResultSetHeader>("INSERT INTO mcp_skill_validation_jobs (id,skill_id,skill_version_id,trigger_type,active_dedupe_key,status,attempts,next_attempt_at,last_error,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)", [job.id, job.skillId, job.skillVersionId, job.trigger, `${job.skillVersionId}:${job.trigger}`, job.status, job.attempts, job.nextAttemptAt, job.lastError, job.createdAt, job.updatedAt]); return result.affectedRows === 1; } catch (error) { if (isDuplicate(error)) return false; throw error; } }
  async findActiveValidationJob(skillVersionId: string, trigger: ValidationTrigger): Promise<SkillValidationJob | null> { const [rows] = await this.executor.query<RowDataPacket[]>("SELECT * FROM mcp_skill_validation_jobs WHERE active_dedupe_key=? AND status IN ('pending','running') LIMIT 1", [`${skillVersionId}:${trigger}`]); return rows[0] ? jobFrom(rows[0]) : null; }
  async listValidationJobs(skillId: string): Promise<SkillValidationJob[]> { const [rows] = await this.executor.query<RowDataPacket[]>("SELECT * FROM mcp_skill_validation_jobs WHERE skill_id=? ORDER BY created_at DESC,id DESC", [skillId]); return rows.map(jobFrom); }
  ...
  async completeValidationJob(jobId: string, workerId: string, at: Date): Promise<boolean> { const [result] = await this.executor.execute<ResultSetHeader>("UPDATE mcp_skill_validation_jobs SET status='completed',active_dedupe_key=NULL,lease_owner=NULL,lease_until=NULL,updated_at=? WHERE id=? AND status='running' AND lease_owner=?", [at, jobId, workerId]); return result.affectedRows === 1; }
  async failValidationJob(jobId: string, workerId: string, nextAttemptAt: Date, maxAttempts: number, errorCode: string): Promise<boolean> { const [result] = await this.executor.execute<ResultSetHeader>("UPDATE mcp_skill_validation_jobs SET status=IF(attempts>=?,'dead','pending'),active_dedupe_key=IF(attempts>=?,NULL,active_dedupe_key),next_attempt_at=?,lease_owner=NULL,lease_until=NULL,last_error=?,updated_at=CURRENT_TIMESTAMP(6) WHERE id=? AND status='running' AND lease_owner=?", [maxAttempts, maxAttempts, nextAttemptAt, errorCode, jobId, workerId]); return result.affectedRows === 1; }
}
