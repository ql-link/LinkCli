...
export const toolView = (projectKey:string, tool:ToolVersion, runtime?:ToolRuntime) => ({ id:tool.id, originalName:tool.originalName, moduleKey:tool.moduleKey, publicName:`${projectKey}__${tool.originalName}`, description:tool.description, inputSchema:tool.inputSchema, outputSchema:tool.outputSchema, riskLevel:tool.riskLevel, runtimeStatus:runtime?.status??"active", suspendedReason:runtime?.suspendedReason??null });
